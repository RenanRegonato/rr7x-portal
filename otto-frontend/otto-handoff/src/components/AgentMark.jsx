/**
 * AgentMark — selo redondo com a inicial do agente.
 * Props: color, initial, size, pulsing
 */
export default function AgentMark({ color = 'peach', initial, size = 'md', pulsing = false }) {
  const palette = {
  ...
  return (
    <div className={`relative grid place-items-center font-display font-medium tracking-tight flex-none ${palette[color]} ${sizes[size]}`}>
      {initial}
      {pulsing && <span className="absolute -inset-1 rounded-xl border-2 border-accent-strong animate-pulse-ring pointer-events-none"/>}
    </div>
  );
}
