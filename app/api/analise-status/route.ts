...
export async function GET(req: NextRequest) {
  ...
  const { data } = await admin
    .from('analises')
    .select('id, status, outputs, nome_ativo, deal_intake, criado_em, regeneracoes_count, facts_extracted_at, consistency_checked_at, mesa_revisao, mesa_revisao_at, risk_correlation_at')
    .eq('id', id)
    .single()
